...
for i=1:length(trks)
    ...
    if curTime>curStart && curEnd>=(curTime+d) && length(trks(i).x)>=(curTime+d)
        if nDimension==2
            
            curX=[trks(i).x((curTime-curStart):(curTime-curStart)+d-1)';trks(i).y((curTime-curStart):(curTime-curStart)+d-1)'];
            curV=[trks(i).x((curTime-curStart)+1:(curTime-curStart)+d)'-trks(i).x((curTime-curStart):(curTime-curStart)+d-1)';trks(i).y((curTime-curStart)+1:(curTime-curStart)+d)'-trks(i).y((curTime-curStart):(curTime-curStart)+d-1)'];
        else
